...
import { ArrowRight, AlertCircle, CheckCircle2, Phone, Mail, Store, User, MapPin, FileText, Lock } from "lucide-react"
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!signupResponse.ok) {
        ...
        throw new Error(err.detail || "Gagal membuat akun pemilik UMKM")
      }

      const userData = await signupResponse.json()

      // 2. Create UMKM associated with the new user
      const umkmResponse = await fetch(`/api/v1/umkm/register?owner_id=${userData.id}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      if (!umkmResponse.ok) {
        throw new Error("Akun dibuat, tapi gagal mendaftarkan UMKM. Silakan hubungi admin.")
      }
            ...
            <div className="flex gap-3 pt-6 border-t">
              ...
              {step < 3 ? (
                <Button
                  type="button"
                  onClick={handleNext}
                  className="rounded-2xl flex-1 gap-2 font-bold"
                  disabled={isSubmitting}
                >
                  Lanjutkan
                ...
                <Button
                  type="submit"
                  className="rounded-2xl flex-1 gap-2 font-bold shadow-lg shadow-primary/20"
                  disabled={isSubmitting}
                >
